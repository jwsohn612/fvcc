...
cvarpyp <- function(ID, W, X = NULL, Z, t, Y, 
  ...
  Li <- map_dbl(Y, function(sign) if(sign>0){rtruncnorm(1, a=0)}else{rtruncnorm(1,b=0)}) %>% as.matrix()
  L_sbj <- split(Li, f=ID)
  
  pu <- r ; D <- diag(r)
  
  g_vec <- rep(n_g_k, K_max); h_vec <- rep(n_h_k, K_max)
  tau <- MCMCpack::rinvgamma(n=K_max, shape=n_g_k/2, scale=n_h_k/2)
  ...
  theta_k <- map(1:K_max, function(k) MASS::mvrnorm(1, mu=rep(0,sum(gamma[[k]])), Sigma = diag(sum(gamma[[k]]))))
  
  W_star <- get_basis_data(tData = W %>% data.matrix, t = t, knot_position = knot_position, sd = sd(t))
  C_star <- do.call(cbind, W_star)
  rm(W_star)
  
  C_star_sbj <- split(C_star %>% data.matrix, f=ID) %>% map( ~ matrix(.x, ncol=dim(C_star)[2]) )
  
  temp <- byproducts(num_obs_per_sbj, K_max, NSbj, C_star_sbj, gamma, X_sbj, beta, Z_sbj, L_sbj, Psi, Psi_0, ID-1, Clusters, active, flatten_gamma = flatten_gamma_with_fixC)
  InvAdj <- temp[[1]]
  InvAdj_0 <- temp[[2]]
  XXi_k <- temp[[3]]
  Xi_k <- temp[[4]]
  
  temp <- MakeRk(K_max, tau, active, C_star_sbj, InvAdj, InvAdj_0, gamma, NSbj, flatten_gamma =flatten_gamma_with_fixC)
  Rk <- temp[[1]]
  ...
  for (iter in 1:num_iters) {
    ...
    gamma <- sample_gamma(ID, K_max, p, active, num_knots, a_pi, b_pi, NSbj, gamma, tau, C_star_sbj, InvAdj, L_sbj, X_sbj, beta, Clusters)
    
    # Draw stick breaking beta
    num_of_ones_K_sbjs <- get_sbj_clusters(K_max, Clusters)
    v <-  c(map_dbl(1:(K_max-1), ~ rbeta(1, 1-lambda+num_of_ones_K_sbjs[.x], nu + .x*lambda+sum(num_of_ones_K_sbjs[(.x+1):K_max]))) ,1)
    ...
    g_vec <- map_dbl(1:K_max, ~ ifelse((.x%in%active), n_g_k, i_g_k))
    h_vec <- map_dbl(1:K_max, ~ ifelse((.x%in%active), n_h_k, i_h_k))
    tau <- map_dbl(1:K_max, ~ MCMCpack::rinvgamma(1, shape=(num_of_ones_vars[.x]+g_vec[.x])/2, scale=h_vec[.x]/2 + (theta_k[[.x]] %*% Rk[[.x]] %*% theta_k[[.x]])/2))
    ...
    probs_Psi <- MakeRkPsi(num_obs_per_sbj, K_max, Rk, tau, active, C_star_sbj, Z_sbj, proposed_Psi,gamma,theta_k,NSbj,flatten_gamma = flatten_gamma_with_fixC)
    ...
    # Draw cluster
    out_mat <- ObtainOutput(NSbj, K_max, Diri_p , L_sbj,C_star_sbj, X_sbj, beta, Z_sbj,bi,gamma,theta_k,flatten_gamma = flatten_gamma_with_fixC)
    ...
    if(iter %% thinning_unit == 0){
      ...
      updated_cluster <- rlist::list.append(updated_cluster, Clusters)
